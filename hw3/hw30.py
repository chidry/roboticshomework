...
des_curr_R=des_R.T@q.R
des_curr_Rinv=np.linalg.inv(des_curr_R)
v=0.5*(des_curr_R-des_curr_Rinv)
def vee_map(v):
    return np.array([v[2,1], v[0,2],v[1,0]])
v=vee_map_(v)
def trace(des_curr_R):
    return des_curr_R[0,0]+des_curr_R[1,1]+des_curr_R[2,2]

v_angle=acos(trace(des_curr_R)-1)/2
...
def normalize(v):
    ...
    retation_error=a_angle @ normalize(v)
